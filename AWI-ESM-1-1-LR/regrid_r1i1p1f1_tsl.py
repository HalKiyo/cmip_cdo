...
def main():
    ...
    remap = Remap(save_flag, variable, realm, model, start_loop, stop_loop, timestep_loop)
    remap.loop()
    inp, _, _ = remap.load(1850, 1850)
    remap.plot(inp)


class Remap():
    def __init__(self, save_flag, variable, realm, model, start_loop, stop_loop, timestep_loop):
